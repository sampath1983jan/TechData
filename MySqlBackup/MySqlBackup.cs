...
namespace MySql.Data.MySqlClient
{
    public class MySqlBackup : IDisposable
    {
      ...
      //  public MySqlCommand Command { get; set; }
        ...
        public MySqlBackup(MySqlCommand cmd,string conn)
        {
            InitializeComponents();
           // Command = cmd;
            Connection = conn;
        ...
        public void ExportToFile(string filePath)
        {
            ...
            taskPath = path + @"\" + Guid.NewGuid().ToString();
        ...
        void ExportStart()
        {
            try
            {
                ...
                var tkbas = Task.Factory.StartNew(() =>
                 {
                     Export_BasicInfo();
                 });
                var tkbdb = Task.Factory.StartNew(() =>
                {
                    Export_CreateDatabase();
                });
                var tkdh = Task.Factory.StartNew(() =>
                {
                    Export_DocumentHeader();
                });
                var tktr = Task.Factory.StartNew(() =>
                {
                    Export_TableRows();
                });
                var tkfn = Task.Factory.StartNew(() =>
                {
                    Export_Functions();
                });
                var tkpr = Task.Factory.StartNew(() =>
                {
                    Export_Procedures();
                });
                var tke = Task.Factory.StartNew(() =>
                {
                    Export_Events();
                });
                var tkv = Task.Factory.StartNew(() =>
                {
                    Export_Views();
                });
                var tktri = Task.Factory.StartNew(() =>
                {
                    Export_Triggers();
                });
                var tkf = Task.Factory.StartNew(() =>
                {
                ...
                {
                    // Task.WaitAll(tkpr);
                    Task.WaitAll(tkbas, tkbdb, tkdh, tktr,
                    tkfn, tkpr, tke, tkv, tktri, tkf);
                    ZipFile.CreateFromDirectory(taskPath, path + @"\" + Guid.NewGuid().ToString() + ".sqldump");

                    //using (var outputStream = File.Create(FilePath))
                    //{
                    //    for (int i = 1; i <= 10; i++) {
                    //        {
                    //            var p = taskPath + @"\bk_" + i + ".sql";
                    //            if (File.Exists(p)) {


                    //                using (var inputStream = File.OpenRead(taskPath + @"\bk_" + i + ".sql"))
                    //                {
                    ...
                    //    }
                    //    outputStream.Close();
                    System.IO.DirectoryInfo di = new DirectoryInfo(taskPath);
        ...
        void Export_InitializeVariables()
        {
           ...
           System.Threading.Tasks.Task[] taskArray = new System.Threading.Tasks.Task[3];
            Dictionary<string,string> dicTables = new Dictionary<string, string>();
            var db = Task<MySqlDatabase>.Factory.StartNew(() =>
            {
                
                _database.GetDatabaseInfo(Connection, ExportInfo.GetTotalRowsMode);
                
                dicTables= Export_GetTablesToBeExported();
                return _database;
                //return result;
            });
            var sr = Task.Factory.StartNew(() =>
            {
                MySqlCommand cmd = new MySqlCommand();
                cmd.Connection = new MySqlConnection(Connection);
                cmd.Connection.Open();
                _server.GetServerInfo(cmd);
        ...
        {
            string fpath = taskPath + @"\bk_1.sql";
            
            using (var wrBasic = new StreamWriter(fpath, false, textEncoding))
            {
                Export_WriteComment(wrBasic, string.Format("MySqlBackup.NET {0}", MySqlBackup.Version));

                if (ExportInfo.RecordDumpTime)
                    Export_WriteComment(wrBasic, string.Format("Dump Time: {0}", timeStart.ToString("yyyy-MM-dd HH:mm:ss")));
                else
                    Export_WriteComment(wrBasic, "");

                Export_WriteComment(wrBasic, "--------------------------------------");
                Export_WriteComment(wrBasic, string.Format("Server version {0}", _server.Version));
                wrBasic.WriteLine();
                wrBasic.Close();
            }           
        }
        ...
        void Export_CreateDatabase()
        {
            ...
            string fpath = taskPath + @"\bk_2.sql";
            
            using (var wrDB = new StreamWriter(fpath, false, textEncoding))
            {
                wrDB.WriteLine();
                wrDB.WriteLine();
                if (ExportInfo.AddDropDatabase)
                    Export_WriteLine(wrDB, String.Format("DROP DATABASE `{0}`;", _database.Name));
                if (ExportInfo.AddCreateDatabase)
                {
                    Export_WriteLine(wrDB,_database.CreateDatabaseSQL);
                    Export_WriteLine(wrDB,string.Format("USE `{0}`;", _database.Name));
                }
                wrDB.WriteLine();
                wrDB.WriteLine();
                wrDB.Close();
        ...
        {
            string fpath = taskPath + @"\bk_3.sql";
            
            using (var wrDoc = new StreamWriter(fpath, false, textEncoding))
            {
                wrDoc.WriteLine();
                MySqlCommand cmd = new MySqlCommand();
                cmd.Connection = new MySqlConnection(Connection);
                cmd.Connection.Open();
                List<string> lstHeaders = ExportInfo.GetDocumentHeaders(cmd);
                cmd.Connection.Close();
                if (lstHeaders.Count > 0)
                {
                    foreach (string s in lstHeaders)
                    {
                        Export_WriteLine(wrDoc, s);
                    }
                    wrDoc.WriteLine();
                    wrDoc.WriteLine();
        ...
        void Export_TBRowsbyfile(int index) {            
            Dictionary<string, string> dicTables = Export_GetTablesToBeExported();
            _totalTables = dicTables.Count;
            if (_totalTables > index) {
                var kvTable = dicTables.ToList()[index];
                string tableName = kvTable.Key;
                string selectSQL = kvTable.Value;
                bool exclude = Export_ThisTableIsExcluded(tableName);
                if (!exclude)
                {
                    string frpath = taskPath + @"\" + tableName.Replace(" ", "_") + ".sql";
                    using (var wrtbrow = new StreamWriter(frpath, false, textEncoding))
                    {
                        Export_Rows(wrtbrow, tableName, selectSQL);
                        wrtbrow.Close();
                    }
                }
            }            
        ...
        void Export_TableRows()
        {
            ...
            {
                string fpath = taskPath + @"\bk_4.sql";

                using (var wrTbrow = new StreamWriter(fpath, false, textEncoding))
                {
                    ...
                    foreach (KeyValuePair<string, string> kvTable in dicTables)
                    {
                        ...
                        if (ExportInfo.ExportTableStructure)
                            Export_TableStructure(wrTbrow, tableName);
                    ...
                    if (ExportInfo.ExportRows)
                    {
                        int length = dicTables.Count;
                        for (int i = 0; i < length; i++) {
                            
                            //var kvTable = dicTables.ToList()[i];
                            if (stopProcess)
                                return;
                            ...
                            var task = new TaskFactory().StartNew(new Action<object>((argu) =>
                            {
                                Export_TBRowsbyfile((int)(argu));
                            }), arg);

                            i = i + 1;
                            //kvTable = dicTables.ToList()[i];
                            //object arg1= kvTable.Key + "#" + kvTable.Value;
                            object arg1 = i;
                            var task_1 = new TaskFactory().StartNew(new Action<object>((argu) =>
                            {
                                Export_TBRowsbyfile((int)(argu));
                            }), arg1);
                            ...
                            var task_2 = new TaskFactory().StartNew(new Action<object>((argu) =>
                            {
                                Export_TBRowsbyfile((int)(argu));
                            }), arg2);                            
                            i = i + 3;                             
                            object arg3 = i;
                            var task_3 = new TaskFactory().StartNew(new Action<object>((argu) =>
                            {
                                Export_TBRowsbyfile((int)(argu));
                            }), arg3);
                            i = i + 4;                             
                            object arg4 = i;
                            var task_4 = new TaskFactory().StartNew(new Action<object>((argu) =>
                            {
                                Export_TBRowsbyfile((int)(argu));
                            }), arg4);
                            Task.WaitAll(task, task_1, task_2, task_3, task_4);
                        }                                          
                    }
        ...
        void Export_TableStructure(TextWriter wr, string tableName)
        {
            if (stopProcess)
                return;

            Export_WriteComment(wr,"");
            Export_WriteComment(wr,string.Format("Definition of {0}", tableName));
            Export_WriteComment(wr, "");

            wr.WriteLine();

            if (ExportInfo.AddDropTable)
                Export_WriteLine(wr, string.Format("DROP TABLE IF EXISTS `{0}`;", tableName));

            if (ExportInfo.ResetAutoIncrement)
                Export_WriteLine(wr, _database.Tables[tableName].CreateTableSqlWithoutAutoIncrement);
            else
                Export_WriteLine(wr, _database.Tables[tableName].CreateTableSql);
        ...
        void Export_Rows(TextWriter tw,string tableName, string selectSQL)
        {
            Export_WriteComment(tw, "");
            Export_WriteComment(tw, string.Format("Dumping data for table {0}", tableName));
            Export_WriteComment(tw, "");
            tw.WriteLine();
            Export_WriteLine(tw, string.Format("/*!40000 ALTER TABLE `{0}` DISABLE KEYS */;", tableName));

            if (ExportInfo.WrapWithinTransaction)
                Export_WriteLine(tw, "START TRANSACTION;");

            Export_RowsData(tw,tableName, selectSQL);

            if (ExportInfo.WrapWithinTransaction)
                Export_WriteLine(tw, "COMMIT;");

            Export_WriteLine(tw, string.Format("/*!40000 ALTER TABLE `{0}` ENABLE KEYS */;", tableName));
            tw.WriteLine();
           // tw.Flush();
        }

        void Export_RowsData(TextWriter tw,string tableName, string selectSQL)
        {
            ...
            {
                Export_RowsData_Insert_Ignore_Replace(tw,tableName, selectSQL);
            }
            else if (ExportInfo.RowsExportMode == RowsDataExportMode.OnDuplicateKeyUpdate)
            {
                Export_RowsData_OnDuplicateKeyUpdate(tw,tableName, selectSQL);
            }
            else if (ExportInfo.RowsExportMode == RowsDataExportMode.Update)
            {
                Export_RowsData_Update(tw,tableName, selectSQL);
            }
        }

        void Export_RowsData_Insert_Ignore_Replace(TextWriter tw,string tableName, string selectSQL)
        {
            ...
            while (rdr.Read())
            {
                ...
                else
                {
                    ...
                    Export_WriteLine(tw, sb.ToString());
                    tw.Flush();
        ...
        void Export_RowsData_OnDuplicateKeyUpdate(TextWriter tw,string tableName, string selectSQL)
        {
            MySqlTable table = _database.Tables[tableName];
        ...
        void Export_RowsData_Update(TextWriter tw,string tableName, string selectSQL)
        {
            MySqlTable table = _database.Tables[tableName];
        ...
        void Export_Procedures()
        {
            ...
            string fpath = taskPath + @"\bk_6.sql";
            
            using (var wrpr = new StreamWriter(fpath, false, textEncoding))
            {
                Export_WriteComment(wrpr,"");
                Export_WriteComment(wrpr, "Dumping procedures");
                Export_WriteComment(wrpr, "");
                wrpr.WriteLine();
                ...
                foreach (MySqlProcedure procedure in _database.Procedures)
                {
                    ...
                    Export_WriteLine(wrpr, string.Format("DROP PROCEDURE IF EXISTS `{0}`;", procedure.Name));
                    Export_WriteLine(wrpr, "DELIMITER " + ExportInfo.ScriptsDelimiter);

                    if (ExportInfo.ExportRoutinesWithoutDefiner)
                        Export_WriteLine(wrpr, procedure.CreateProcedureSQLWithoutDefiner + " " + ExportInfo.ScriptsDelimiter);
                    else
                        Export_WriteLine(wrpr, procedure.CreateProcedureSQL + " " + ExportInfo.ScriptsDelimiter);

                    Export_WriteLine(wrpr, "DELIMITER ;");
                    wrpr.WriteLine();
                }
                wrpr.Flush();
                wrpr.Close();
        ...
        void Export_Functions()
        {
            if (!ExportInfo.ExportFunctions || _database.Functions.Count == 0)
                return;
            string fpath = taskPath + @"\bk_5.sql";
            
            using (var wrfn = new StreamWriter(fpath, false, textEncoding))
            {
                Export_WriteComment(wrfn,"");
                Export_WriteComment(wrfn, "Dumping functions");
                Export_WriteComment(wrfn, "");
                wrfn.WriteLine();
                ...
                foreach (MySqlFunction function in _database.Functions)
                {
                    ...
                    Export_WriteLine(wrfn, string.Format("DROP FUNCTION IF EXISTS `{0}`;", function.Name));
                    Export_WriteLine(wrfn, "DELIMITER " + ExportInfo.ScriptsDelimiter);

                    if (ExportInfo.ExportRoutinesWithoutDefiner)
                        Export_WriteLine(wrfn, function.CreateFunctionSQLWithoutDefiner + " " + ExportInfo.ScriptsDelimiter);
                    else
                        Export_WriteLine(wrfn, function.CreateFunctionSQL + " " + ExportInfo.ScriptsDelimiter);

                    Export_WriteLine(wrfn, "DELIMITER ;");
                    wrfn.WriteLine();
                }
        ...
        void Export_Views()
        {
            ...
            string fpath = taskPath + @"\bk_8.sql";
            
            using (var wrvw = new StreamWriter(fpath, false, textEncoding))
            {
                // ReArrange Views
                ...
                Export_WriteComment(wrvw,"");
                Export_WriteComment(wrvw, "Dumping views");
                Export_WriteComment(wrvw, "");
                wrvw.WriteLine();
                ...
                foreach (var viewname in lst)
                {
                    ...
                    Export_WriteLine(wrvw, string.Format("DROP TABLE IF EXISTS `{0}`;", view.Name));
                    Export_WriteLine(wrvw, string.Format("DROP VIEW IF EXISTS `{0}`;", view.Name));

                    if (ExportInfo.ExportRoutinesWithoutDefiner)
                        Export_WriteLine(wrvw, view.CreateViewSQLWithoutDefiner);
                    else
                        Export_WriteLine(wrvw, view.CreateViewSQL);
        ...
        void Export_Events()
        {
            ...
            string fpath = taskPath + @"\bk_7.sql";
            
            using (var wrevn = new StreamWriter(fpath, false, textEncoding))
            {
                Export_WriteComment(wrevn,"");
                Export_WriteComment(wrevn, "Dumping events");
                Export_WriteComment(wrevn, "");
                wrevn.WriteLine();
                ...
                foreach (MySqlEvent e in _database.Events)
                {
                    ...
                    Export_WriteLine(wrevn, string.Format("DROP EVENT IF EXISTS `{0}`;", e.Name));
                    Export_WriteLine(wrevn, "DELIMITER " + ExportInfo.ScriptsDelimiter);

                    if (ExportInfo.ExportRoutinesWithoutDefiner)
                        Export_WriteLine(wrevn, e.CreateEventSqlWithoutDefiner + " " + ExportInfo.ScriptsDelimiter);
                    else
                        Export_WriteLine(wrevn, e.CreateEventSql + " " + ExportInfo.ScriptsDelimiter);

                    Export_WriteLine(wrevn, "DELIMITER ;");
                    wrevn.WriteLine();
                }
        ...
        void Export_Triggers()
        {
            if (!ExportInfo.ExportTriggers || _database.Triggers.Count == 0)
                return;
            string fpath = taskPath + @"\bk_9.sql";
            
            using (var wrt = new StreamWriter(fpath, false, textEncoding))
            {
                Export_WriteComment(wrt,"");
                Export_WriteComment(wrt, "Dumping triggers");
                Export_WriteComment(wrt, "");
                wrt.WriteLine();
                ...
                foreach (MySqlTrigger trigger in _database.Triggers)
                {
                    ...
                    Export_WriteLine(wrt, string.Format("DROP TRIGGER /*!50030 IF EXISTS */ `{0}`;", trigger.Name));
                    Export_WriteLine(wrt, "DELIMITER " + ExportInfo.ScriptsDelimiter);

                    if (ExportInfo.ExportRoutinesWithoutDefiner)
                        Export_WriteLine(wrt, trigger.CreateTriggerSQLWithoutDefiner + " " + ExportInfo.ScriptsDelimiter);
                    else
                        Export_WriteLine(wrt, trigger.CreateTriggerSQL + " " + ExportInfo.ScriptsDelimiter);

                    Export_WriteLine(wrt, "DELIMITER ;");
                    wrt.WriteLine();
                }
        ...
        {
            string fpath = taskPath + @"\bk_10.sql";
            
            using (var wrf = new StreamWriter(fpath, false, textEncoding))
            {
                wrf.WriteLine();
                ...
                if (ExportInfo.RecordDumpTime)
                {
                    ...
                    wrf.WriteLine();
                    wrf.WriteLine();
                    Export_WriteComment(wrf,string.Format("Dump completed on {0}", timeEnd.ToString("yyyy-MM-dd HH:mm:ss")));
                    Export_WriteComment(wrf,string.Format("Total time: {0}:{1}:{2}:{3}:{4} (d:h:m:s:ms)", ts.Days, ts.Hours, ts.Minutes, ts.Seconds, ts.Milliseconds));
                }
        ...
        void Export_WriteComment(TextWriter tw,string text)
        {
            Export_WriteLine(tw,string.Format("-- {0}", text));
        }

        void Export_WriteLine(TextWriter tw,string text)
        {
        ...
        void Import_InitializeVariables()
        {
            ...
            _currentBytes = 0L;
            _sbImport = new StringBuilder();
            _mySqlScript = new MySqlScript(cmd.Connection);
