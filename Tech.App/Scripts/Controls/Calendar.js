...
(function (factory) {
    ...
    Calendar.prototype.drawEventsOrDaynotes = function (e, index, start, end, title, content, category, color, time_interval, classes) {
     ...
     //   li.addClass("draggable");
        ...
        interact('#data' + index)
            .draggable({
                onmove: function (event) { window.dragMoveListener(event,self); },
                modifiers: [
            ...
            .on('resizemove', function (event) {
                ...
                var dst = $(target).attr("data-start");
                var det = $(target).attr("data-end");

                var dataStart = moment.unix(dst).toDate();
                var dataEnd = moment.unix(det).toDate();
    ...
    function dragMoveListener(event,self) {
        var target = event.target
        ...
        console.log("left:" + p.left + "- top:" + top);
       
        var totalmin = (Math.floor(((top+ 2) / self.conf.weekday.timeline.heightPx)) * self.conf.weekday.timeline.intervalMinutes);
        var hr = Math.floor(totalmin / 60);
        var rmin = ((((top + 2) / self.conf.weekday.timeline.heightPx)) * self.conf.weekday.timeline.intervalMinutes);
        var min = Math.floor(rmin % 60);
        hr = parseInt( self.conf.weekday.timeline.fromHour )+ hr;
        console.log("hr: " + hr + "min: " + min);

        totalmin = 0;
        rmin = 0;
        totalmin = Math.floor(((height+2) / self.conf.weekday.timeline.heightPx)) * self.conf.weekday.timeline.intervalMinutes;
        var ehr = Math.floor(totalmin / 60);
        rmin = (((height + 2) / self.conf.weekday.timeline.heightPx)) * self.conf.weekday.timeline.intervalMinutes;
        var emin = Math.floor(rmin % 60);
        ehr = parseInt(self.conf.weekday.timeline.fromHour) + ehr;
        ...
        var newstart = new Date(dataStart.getFullYear(), dataStart.getMonth(), dataStart.getDate(), hr, min);
        var newend = new Date(dataEnd.getFullYear(), dataEnd.getMonth(), dataEnd.getDate(), ehr, emin);
     
        $(target).attr("data-start", moment(newstart).format("X"));
        $(target).attr("data-end", moment(newend).format("X"));
        //var duration = det - dst;
        //debugger;
        //eventHeight = (duration / 60 / self.conf.weekday.timeline.intervalMinutes * self.conf.weekday.timeline.heightPx) + 1;
        //console.log("height time:" +eventHeight)
    ...
    Calendar.prototype.addBtnLeftRight = function () {
        ...
        self.element.find('.btn-move-calendar').click(function (event) {
